...
unsigned int bondValue(BondType bt) {
	...
	case BondType::Aromatic:
		std::cout << "Aromatic bonds are not supported." << std::endl;
		assert(false);
		std::exit(1);
		break;
	}
...
std::vector<std::shared_ptr<mod::rule::Rule> > doStuff(const std::vector<std::shared_ptr<mod::graph::Graph> > &educts,
	...
	for(const VertexMap &vm : vertexMaps) {
		...
		const auto iter = std::find_if(rulesInternal.begin(), rulesInternal.end(), [&r](const std::unique_ptr<mod::lib::Rules::Real> &rOther) {
			auto labelSettings = mod::LabelSettings(mod::LabelType::String, mod::LabelRelation::Isomorphism);
...
int main(int argc, char **argv) {
	...
	{ // do something else, e.g., take SMILES strings from argv
        ...
        g3 = mod::graph::Graph::smiles("C=O", "C=O");
...
std::unique_ptr<mod::lib::Rules::Real> createRule(const VertexMap &vertexMap,
	...
	{ // error checking
		...
		for(const auto &vt : vertexMap) {
			...
			if(lLabel != rLabel) {
				IO::log() << "Label mismatch: " << lLabel << " " << vLeftId << " <=> " << vRightId << " " << rLabel << std::endl;
				assert(false);
				std::exit(1);
			}
		}
	...
	if(doChemistryCheck) {
		...
		for(const auto v : asRange(vertices(core))) {
			...
			if(bondChange != 0) {
				IO::log() << "Non-zero bond change for vertex " << get(boost::vertex_index_t(), core, v) << ": " << bondChange << "\n";
				IO::log() << "Label: (" << get(pStringCore.getLeft(), v) << ", " << get(pStringCore.getRight(), v) << ")" << std::endl;
				error();
			}
			if(left > 2) {
				IO::log() << "Too many left side edges (" << left << ") for vertex " << get(boost::vertex_index_t(), core, v) << ".\n";
				IO::log() << "Label: (" << get(pStringCore.getLeft(), v) << ", " << get(pStringCore.getRight(), v) << ")" << std::endl;
				error();
			}
			if(right > 2) {
				IO::log() << "Too many right side edges (" << right << ") for vertex " << get(boost::vertex_index_t(), core, v) << ".\n";
				IO::log() << "Label: (" << get(pStringCore.getLeft(), v) << ", " << get(pStringCore.getRight(), v) << ")" << std::endl;
				error();
			}
		}
